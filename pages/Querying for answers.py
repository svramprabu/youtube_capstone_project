    ...
    try:
        ...
        if mydb.is_connected():
            ...
            if st.button(" Q1 What are the names of all the videos and their corresponding channels?"):
                Q1="SELECT video_det.video_title, playlist_det.Channel_title FROM video_det LEFT JOIN playlist_det ON video_det.playlist_id=playlist_det.playlist_id"
                ans1 = pd.read_sql(Q1,mydb)
                st.write(f"Query: {Q1}")
                st.write(ans1)

            if st.button("Q2 Which channels have the most number of videos, and how many videos do they have?"):
                Q2="SELECT * FROM sql12618369.channel_det ORDER BY Number_of_Videos DESC"
            ...
            if st.button("Q3 What are the top 10 most viewed videos and their respective channels?"):
                ...
                for each_channel in channels_list:
                    Q3=f"SELECT * FROM sql12618369.video_det WHERE channelTitle='{each_channel}' ORDER BY viewCount DESC LIMIT 10"
